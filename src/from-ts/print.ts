...
const printArray = (ast: ArrayAst, options: Options): string =>
  ast.item.type === 'union'
    ? `[${ast.item.items.map(i => printAny(i, options))}]`
    : `[${printAny(ast.item, options)}]`;

const printFunction = (ast: FunctionAst, options: Options): string =>
  ast.key === 'ref'
    ? options.quote
      ? `'${ast.name}'`
      : `${ast.name}`
...
const printObject = (ast: ObjectAst, options: Options): string =>
...
const printUnion = (ast: UnionAst, options: Options): string => {
...
const printAny = (ast: Ast, options: Options): string => {
  switch (ast.type) {
...
export const print = (items: Ast[], options: Options) => `${
